...
Assuming a narrow Gaussian distribution for the source term $\varphi_\mathrm{imp}$, the mobile particles concentration profile can be approximated \textcolor{black}{by a triangular shape \sidecite{schmid_diffusion-trapping_2016} (see Figure \ref{fig:recomb sketch})}.
...
Due to the numerous processes that are thermally activated, it is essential to model heat transfers.
...
FESTIM is based on the Finite Element Method to solve this set of equations and boundary conditions and the python package FEniCS \sidecite{alnaes_fenics_2015} was employed.
...
\begin{equation}
    u = \sum^N_{i=0}c_i \phi_i(x, y, z)
    \label{eq: FEM solution}
\end{equation}
where $c_i$ are the coefficient to be determined (called degrees of freedom) and $\phi_i$ are polynomials.
...
%
% maybe explain why multiplying by a test function
%
...
Equation \ref{eq: weak form 2} then gives rise to a system of $N+1$ equations that can be solved to determine the coefficients $c_i$ in Equation \ref{eq: FEM solution}.
