...
The numerical scheme to simulate these models and its implementation in FESTIM are also presented alongside with an introduction to the finite element method.
Finally, the verification \& validation of the FESTIM code is performed to guaranty its reliability.
...
\subsubsection{Analytical simplification for implanted sources of H}
...
$\varphi_\mathrm{recombination}$ can also be expressed as a function of the recombination coefficient $K$:

\begin{eqnarray}
    \varphi_\mathrm{recombination} &= K c_{0}^{2} = \varphi_\mathrm{imp}\\
...
Such a discretisation can be easily done for 1D simulations but is very complex and often makes the mesh very large which increases drastically the computational cost.
...
All these equations are solved with the finite element code FESTIM.
Thanks to the verification \& validation process undergone with FESTIM, it was shown that (1) the governing equations were correctly solved and (2) these equations can represent real life processes.
