...
Other designs aim at demonstrating \gls{plasma} gain (\textit{ie} $Q > 1$) sooner than \acrshort{iter} and at a smaller scale (see \reffig{comparison reactors}).
...
In most tokamaks, the escaping \gls{plasma} is diverted towards a dedicated component that is heat-resistant.
...
To this end, the neutrons of the DT fusion reactions will be harnessed in a component containing \gls{Li} surrounding the \gls{plasma} called the \emph{\glspl{breeding blanket}} (see \reffig{blanket_shimwell}).
...
If its size is big enough the cluster pressure is sufficient to knock off a \gls{W} atom from the lattice, creating a \gls{W} \gls{vacancy} and an interstitial \gls{W} atom (a \gls{Frenkel pair}).
...
Once a bubble nucleus is created via \gls{trap mutation}, it can continue to grow via three main mechanisms: absorption of clusters, loop punching or blistering.
...
During the punching event, a whole facet of \gls{W} atoms is pushed and the vacant lattice sites are absorbed by the bubble allowing the bubble to expand and reducing the pressure in it \sidecite{sefta_surface_2013}.
...
\Glspl{dislocation loop} happen at very high pressure and if the number of vacancies in the lattice is low compared to the amount of He atoms.
...
This is why He \gls{bursting} is characterised by sharp drops in the He inventory \sidecite{hammond_helium_2019}.
...
However, if these structures were to be removed during \gls{plasma} operation via erosion, W atoms could be fed into the \gls{plasma}, affecting the tokamak performances.
...
Takamura \textit{et al} showed \gls{fuzz} could be grown under relevant tokamak conditions (high-flux He \gls{plasma} irradiation and surface temperature greater than \SI{1250}{K}) \sidecite{takamura_formation_2006}.
Baldwin \textit{et al} showed the \gls{fuzz} thickness evolved as a square root of the fluence \cite{baldwin_effects_2009}.
...
They identified different \gls{fuzz} growth regimes depending on the He fluence due to the change in porosity of the fuzzy layer during the growth process.
...
The same phenomenon has been observed by Miyamoto \textit{et al} \sidecite{miyamoto_microscopic_2011} which contributes to reducing \gls{H} retention.
...
This suggests that observed reduction or increase of D retention in mixed H-He \gls{plasma} experiment are depending on the depth of implantation.
...
He bubbles were found in the near surface region (see \reffig{he bubbles ialovega}) and significantly different TDS spectra were observed after several \gls{H} implantations/desorptions (see \reffig{TDS example ialovega}).
...
To this end, a \gls{festim} model of ITER-like monoblock will be developped.
...
The inventory of the \acrshort{iter} and WEST \glspl{divertor} will be computed for several \gls{plasma} scenarios.
...
This helium transport code was then coupled to \gls{festim} to estimate the potential impact of helium on the tritium inventory estimations.
\end{itemize}
