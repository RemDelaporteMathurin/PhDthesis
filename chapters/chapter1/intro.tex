...
Since each coil has a unique shape, the cost of manufacturing such a reactor is higher than tokamaks for which coils can be manufactured in series.
...
Assuming energy equilibrium (\textit{ie} $\frac{\partial W}{\partial t} = 0$), \refeq{plasma energy balance} can therefore be written as:
\begin{align}
...
The record of $Q = 0.68$ by the European tokamak \gls{jet} and was performed in 1997 \sidecite{mailloux_overview_2022}.
The objective of the \acrshort{iter} tokamak, currently under construction in France, is to demonstrate an amplification factor of $Q=10$ over \SI{400}{s} \sidecite{casper_development_2013}. 
...
Other designs aim at demonstrating plasma gain (\textit{ie} $Q > 1$) sooner than \acrshort{iter} and at a smaller scale (see \reffig{comparison reactors}).
...
Using \gls{cfc} also increased the plasma performances greatly \sidecite{federici_plasma-material_2001}.
...
The X-divertor has two strike points on the inner and outer targets (see \reffig{divertor diagram}).
...
The inner and outer strike points (respectively \acrshort{isp} and \acrshort{osp}) will be located on the \gls{ivt} and the \gls{ovt}.
...
To this end, the neutrons of the DT fusion reactions will be harnessed in a component containing \gls{Li} surrounding the plasma called the \emph{\glspl{breeding blanket}} (see \reffig{blanket_shimwell}).
...
Tritium transport in \glspl{breeding blanket} is crucial as trapped tritium in solid parts would increase the startup inventory.
For a given doubling time, a higher startup inventory means a higher \gls{tbr} requirement.
The blanket inventory in the solid parts can be reduced by (1) reduce the amount of solid components (this will also increase the \gls{tbr} \cite{shimwell_multiphysics_2019}) (2) designing permeation barriers \sidecite{utili_design_2022}.
...
In \acrshort{iter} for instance, the in-vessel inventory of tritium is limited to \SI{1}{kg} including \SI{120}{g} retained in cryo-pumps \sidecite{roth_tritium_2008}.
...
This is also the case for the \gls{divertor} region where recycling can occur \sidecite{denis_dynamic_2019, causey_hydrogen_2002}.
...
Since tritium decays into helium-3 (see \refeq{tritium decay}), regions with high tritium retention are expected to act as a source of helium over time \cite{shimada_tritium_2017}.
...
Diffusion can also be assisted by temperature gradients (called the \textit{Soret effect} or \textit{thermophoresis}) \sidecite{martinez_thermal_2021, hodille_estimation_2017, longhurst_soret_1985} or hydrostatic pressure gradients.
...
This modelling technique was used to estimate the diffusion coefficient of \gls{H} in W \sidecite{wang_molecular_2020, zhou_molecular_2016,kato_super-saturated_2015,liu_hydrogen_2014} and for He in W \sidecite{faney_numerical_2013,faney_spatially_2014,faney_spatially_2015,sefta_surface_2013,perez_mobility_2017}.

% Experiments
Diffusivity of hydrogen has also been measured experimentally in W \sidecite{holzner_solute_2020, frauenfelder_solution_1969, anderl_hydrogen_1990}, copper and copper alloys (CuCrZr) \sidecite{anderl_deuterium_1992} and other metals.
...
TDS experiments consist in loading a metal sample with the studied species (\textit{eg} \gls{H} or \gls{He}) and heat it at different temperatures with a well controlled temperature ramp (\textit{eg} \SI{1}{K.s^{-1}}, \SI{10}{K.s^{-1}}...) while measuring the desorption flux.
...
Similarly, the binding energy of a hydrogen atom in a mono-vacancy varies from \SI{0.5}{eV} (with six hydrogen atoms trapped) to \SI{1.3}{eV} (empty vacancy) (see \reffig{trapping energy hydrogen in tungsten}).
...
This phenomena, called "clustering", was highlighted by \gls{dft} studies \cite{becquart_density_2009,dunn_rate_2013} and MD simulations \cite{henriksson_molecular_2006}.
...
Clustering of \gls{H} atoms is less clear and Henriksson \textit{et al} showed that \gls{H} atoms do not form bonds with other \gls{H} atoms in bcc W \sidecite{henriksson_difference_2005}.
...
Trap mutation has been modelled in W using \gls{dft} \sidecite{boisse_modelling_2014} and Monte Carlo computations \sidecite{de_backer_modeling_2015}.
...
It is considered immobile since it would require either diffusion of another vacancy next to it, or recombination of the Frenkel pair in order to diffuse \sidecite{morishita_nucleation_2007}.
...
An effort has been made by Ialovega to measure the pressure inside helium bubbles using EELS (Electron Energy Loss Spectroscopy) \sidecite{ialovega_surface_2021}.
...
Blistering usually happens at low temperatures because only then the growth rate of the bubble is greater than the dissolution in the bulk (which depends on the thermally activated diffusion coefficient and solubility).
...
When a bubble grows near the surface and is over-pressurised, bursting can occur (see \reffig{bubble bursting zhou}).
...
These nanostructures are made of W tendrils (see \reffig{w fuzz wright}).
...
Takamura \textit{et al} showed fuzz could be grown under relevant tokamak conditions (high-flux He plasma irradiation and surface temperature greater than \SI{1250}{K}) \sidecite{takamura_formation_2006}.
Baldwin \textit{et al} showed the fuzz thickness evolved as a square root of the fluence \cite{baldwin_effects_2009}.
...
McCarthy \textit{et al} studied the formation of W fuzz (see \reffig{w fuzz mccarthy}) at helium fluences ranging from \SI{4e23}{m^{-2}} to \SI{e25}{m^{-2}}, temperatures ranging from \SI{1050}{K} to \SI{1150}{K} and He ion energies from \SI{80}{eV} to \SI{100}{eV}.
...
He bubbles were found in the near surface region (see \reffig{he bubbles ialovega}) and significantly different TDS spectra were observed after several H implantations/desorptions (see \reffig{TDS example ialovega}).
This works gives evidence of an interaction between He and H in W.

\section{Problem definition}

The main focus of this PhD work is the estimation of H \glspl{isotope} retention and permeation in tokamaks.
...
For all these reasons, the FESTIM code was developed and used to conduct this PhD research.
...
The next stage is to scale up from a monoblock model to a full \gls{divertor} model.
...
The inventory of the \acrshort{iter} and WEST \glspl{divertor} will be computed for several plasma scenarios.
