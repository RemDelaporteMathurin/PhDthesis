...
divHretention can directly link \gls{solps}/\gls{soledge} output files and produce a distribution of \gls{monoblock} \gls{inventory} as in \reffig{behaviour law example}.
...
Even though cycling can have an effect on \gls{H} outgassing at the \gls{monoblock} plasma facing surface, it was shown in \sidecite{hodille_modelling_2021} that the evolution of the \gls{monoblock} \gls{inventory} with the fluence was not affected.
...
The maximum hydrogen \gls{inventory} in the \gls{iter} \gls{divertor} was approximately \SI{14}{g} after \SI{e7}{s} of continuous plasma exposure (25 000 \gls{iter} discharges), which is well below the safety limit (\SI{1}{kg}).
