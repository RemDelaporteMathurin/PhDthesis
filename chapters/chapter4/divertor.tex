...
The monoblock behaviour law proposed in Chapter \ref{Chapter3} was used to estimate fuel retention in the divertors of WEST and ITER.
...
The maximum hydrogen inventory in the ITER divertor was $\approx \SI{14}{g}$ after \SI{e7}{s} of continuous plasma exposure.
This is well below the safety limit (\SI{1}{kg}).
Moreover, since the behaviour law is based on 2D monoblock simulations, this value is a upper estimate.
2D simulations are indeed conservative in terms of inventory (see \ref{3D edge effects}).
