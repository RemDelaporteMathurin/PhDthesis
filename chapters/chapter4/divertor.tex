...
Inputs are taken from SolEdge3X-EIRENE \sidecite{bufferand_three-dimensional_2019} and SOLPS-ITER \sidecite{kaveeva_solps-iter_2020} plasma simulations.
...
The monoblock behaviour law proposed in Chapter \ref{Chapter3} was used to estimate ruel retention in both the WEST and the ITER divertors.
...
The underlying monoblock model has also a few limitations, as detailed in Chapter \ref{Chapter3}.
First, the set of trapping parameters that was used may not be relevant for every region of the divertor.
These properties can however be estimated from further experimental work.
...
3D edge effects due to desorption from the gaps between the monoblocks would decrease the estimated inventory.
The current assumption is therefore conservative and represents a "worst-case scenario" where no desorption from the gaps is assumed.
