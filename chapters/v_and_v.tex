...
This verification case aims at checking the FESTIM code is correctly solving the governing Equations \ref{eq: c/s conservation}, \ref{eq:mobile} and \ref{eq: diffusion equation changed}.
...
It is worth noting that, when choosing a manufactured solution, one must ensure it satisfies all the governing equations (especially \ref{eq: c/s conservation}).
