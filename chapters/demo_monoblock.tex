...
The DEMO monoblock geometry differs slightly from the ITER geometry (see \reffig{geometry DEMO monoblock}) but the general concept is the same, meaning the observations made in this Section are also valid for the ITER geometry.
...
\begin{figure}
    ...
    \labfig{geometry DEMO monoblock}
\end{figure}

The boundary conditions for the steady state heat transfer problem are the same as for the 2D case (see \refeq{bc thermal DEMO monoblock}).
The boundary conditions for the transient H transport problem are similar (see \refeq{bc H transport DEMO monoblock}).
A non-homogeneous mobile concentration is assumed at the plasma exposed surface to simulate an implanted source of particles (see \refsec{triangle model}).
...
Two intrinsic traps were set in W, one trap in the Cu interlayer and two traps in the CuCrZr cooling pipe (see \reftab{traps monoblock DEMO}).
...
The temperature field obtained was very similar to the 2D case (see \reffig{T field 3D monoblock}) with a top surface temperature of approximately \SI{1200}{K}.
...
\begin{figure} [h]
    ...
    \labfig{T field 3D monoblock}
\end{figure}


% Retention fields
As expected, a higher retention was observed in the case without desorption (see \reffig{retention fields 3D monoblocks}).
...
\begin{figure} [h]
    ...
    \labfig{retention fields 3D monoblocks}
\end{figure}

% Inventory
The total H inventory in the monoblock was also between one and three orders of magnitude lower in the case with desorption (see \reffig{inventory vs time DEMO monoblock}).
...
The simulations performed in \refsec{influence of exposure conditions} then overestimate the monoblock H inventory.
...
\begin{figure} [h]
    ...
    \labfig{inventory vs time DEMO monoblock}
\end{figure}

% fluxes
These 3D simulations are however essential to estimate the outgassing fluxes from the poloidal gaps.
The particle flux at the poloidal gap is six times higher than the flux at the toroidal gap  (see \reffig{fluxes DEMO monoblock}).
...
% inventory
As the thickness increases, the inventory per unit thickness increases for the case with instantaneous recombination on the poloidal gap (see \reffig{inventory vs time DEMO monoblock}).
It remains constant for the case with no recombination since it is equivalent to a 2D case (the decrease observed at low thicknesses is due to the impact of the CuCrZr pipe between monoblocks as shown on \reffig{geometry DEMO monoblock}).
The relative difference between the cases with or without recombination on the poloidal gap decreases as the thickness increases (see \reffig{ratio 3D thickness monoblock}).
...
\begin{figure} [h]
    ...
    \labfig{ratio 3D thickness monoblock}
\end{figure}

% flux
The permeation flux towards the cooling channel per unit thickness globally increases with the monoblock thickness (see \reffig{flux to coolant DEMO monoblock}).
...
\begin{figure} [h]
    ...
    \labfig{flux to coolant DEMO monoblock}
\end{figure}
...
Without this extrusion, the case without recombination on the poloidal gap corresponds to a pure 2D case and the inventory is independent of the thickness (see \reffig{inventory monoblock vs thickness no gap}).
...
\begin{figure} [h]
    ...
    \labfig{inventory monoblock vs thickness no gap}
\end{figure}
