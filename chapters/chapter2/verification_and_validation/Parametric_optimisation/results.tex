...
The TDS spectrum measured by Ogorodnikova et al. \sidecite{ogorodnikova_deuterium_2003} has been reproduced by setting all traps parameters as free parameters.
The fitting procedure has been run for several numbers of traps as shown on \reffig{number of traps comparison}.
...
The resulting fit is shown on \reffig{5D TDS} alongside with the contribution of each trap to the total spectrum.
An interesting feature of this spectrum is the negative area of the contribution of the second trap around \SI{400}{K}.
Because not all of these traps are saturated, when trap 1 is empyting, some hydrogen particles are nearly instantly trapped in the second trap which has a higher detrapping energy.
...
\begin{figure*} [h!]
    \centering
        \begin{subfigure}[t]{0.5\linewidth}
            ...
            \labfig{number of traps comparison}
        \end{subfigure}%
        \begin{subfigure}[t]{0.5\linewidth}
            \centering
            \captionsetup{width=.9\linewidth}
...
% Hollingsworth et al. performed thermo-desorption on pre-damaged EUROFER at several damage levels \sidecite{hollingsworth_comparative_2019}.
% Three spectra with similar exposure conditions have been fitted with one trapping site (since only one peak appears on the spectra) as shown on \reffig{TDS EUROFER}.
...
% A more thorough study of these experiments could involve constraining the algorithm with profilometry data obtained by Hollingsworth et al. \cite{hollingsworth_comparative_2019}.
...
% The experiment performed on Aluminium by Quiros et al. \sidecite{quiros_blistering_2019, quiros_blister_2017} has also been reproduced with FESTIM.
...
% The resulting simulated TDS spectrum is shown on \reffig{TDS alu}.
% \begin{figure} [h!]
%     \centering
...
% However, the TDS spectrum obtained with this procedure better fits the experimental data since the one obtained by Quiros et al. requires a 10-fold increase.
...
% Be co-deposition experiments performed by Baldwin et al. \sidecite{baldwin_experimental_2014} were reproduced.
% In this experiment, a \SI{1}{\micro m} thick Be-D layer is grown on Tungsten at \SI{330}{K}. 
% Following the strategy proposed by Baldwin et al., only the thermo-desorption phase has been simulated with two trapping sites with homogeneously distributed densities and with initial occupancies $f_i$.
...
% The resulting optimised TDS spectrum is shown on \reffig{TDS baldwin}.
...
% These values are in agreement with the ones found by Baldwin et al. \cite{baldwin_experimental_2014} and took only a few of minutes to compute since the implantation phase was not simulated.
...
As shown on \reffig{number of traps comparison} too few traps in the simulation will not result in a satisfactory fit (even though the optimisation routine will converge to an optimised solution).
Moreover, as shown on \reffig{hurley_comparison}, one single TDS spectrum can be reproduced with several traps of different energies and densities.
...
Adding traps to an optimisation problem can also help to have a better fit of the experimental data in some cases.
...
\begin{figure}[h!]
    ...
    \labfig{hurley_comparison}
\end{figure}

In the first case with only one trapping site, as described by Hurley et al. in \sidecite{hurley_numerical_2015}, the binding energy is \SI{0.55}{eV} and the trap density is \SI{2.08e24}{m^{-3}}.
