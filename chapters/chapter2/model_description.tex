...
The mobile particles concentration profile can be approximated by a triangular shape \sidecite{schmid_diffusion-trapping_2016} (see \reffig{recomb sketch}).
...
Finally, to model a convective heat flux when the surface is in contact with a fluid (\textit{eg} cooling pipes, natural convection...), a Robin boundary condition needs to be employed (see \refeq{convective bc T}).
\begin{equation}
...
The code \gls{festim} \sidecite{delaporte-mathurin_finite_2019} was therefore developped in order to numerically solve these coupled equations.
