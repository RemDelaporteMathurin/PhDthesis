...
The code FESTIM \sidecite{delaporte-mathurin_finite_2019} was therefore developped in order to numerically solve these coupled equations.
...
FESTIM provides an even higher level of abstraction than \gls{fenics} by providing a user-friendly interface dedicated to H transport and heat transfer problems.
...
% physics
As mentioned above, FESTIM simulates hydrogen transport (diffusion and trapping) and additional physics can be incorporated, such as the Soret effect (even though not used in this work) and conservation of chemical potential at interfaces...
...
% visualisation
Similarly, FESTIM (\gls{fenics}) visualisation functions are limited.
...
All these equations are solved with the newly developed finite element code FESTIM, which heavily relies of \gls{fenics}.
