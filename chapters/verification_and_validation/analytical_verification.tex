...
Both methods consist in comparing a computed solution with an analytical solution and measuring the error.
The analytical solution in the MES is obtained by solving the governing equations.
With the MMS, on the other hand, the problem is reversed: an arbitrary analytical solution (called manufactured solution) is chosen and injected in the governing equations.
It is then possible to determine source terms and boundary conditions.
These are then fed into the code and the computed solution is compared to the manufactured (analytical) solution.
